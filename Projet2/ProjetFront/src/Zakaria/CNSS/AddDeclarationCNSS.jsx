...
const TAUX_SALARIAL_PLAFONNE = 0.0429;
const TAUX_SALARIAL_DEPLAFONNE = 0.0226;
const TAUX_PATRONAL_PLAFONNE = 0.0898;
const TAUX_PATRONAL_DEPLAFONNE = 0.1211;

const calculerTotalCotisations = (salaireBrut) => {
  const salaire = Number(salaireBrut || 0);
  const basePlafonnee = Math.min(salaire, CNSS_PLAFOND);
  const cotisationSalarie = (basePlafonnee * TAUX_SALARIAL_PLAFONNE) + (salaire * TAUX_SALARIAL_DEPLAFONNE);
  const cotisationPatronale = (basePlafonnee * TAUX_PATRONAL_PLAFONNE) + (salaire * TAUX_PATRONAL_DEPLAFONNE);

  return cotisationSalarie + cotisationPatronale;
};
...
function AddDeclarationCNSS({ toggleDeclarationForm, onDeclarationSaved = () => { }, selectedDeclaration = null }) {
  ...
  const montantTotal = useMemo(
    () => selectedEmployees.reduce((sum, employee) => sum + calculerTotalCotisations(employee.salaire), 0),
    [selectedEmployees]
  );
