...
function SanctionManager() {
  ...
  // Préchargement des employés pour SanctionTable
  ...
  // Référence vers le composant SanctionTable
  ...
  useEffect(() => {
    setTitle("Sanctions disciplinaires");
  ...
  useEffect(() => {
    if (editingDepartement && editInputRef.current) {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: error.response
          ...
          : "Une erreur s'est produite lors de l'ajout du sous-département.",
        confirmButtonText: 'OK',
      });
    }
  };
  ...
  const getSubDepartmentIds = useCallback((departments, id) => {
    ...
    const findDepartmentInTree = (depts, targetId) => {
    ...
    const targetDept = findDepartmentInTree(departments, id);
    if (targetDept) {
      addIds(targetDept);
    }
