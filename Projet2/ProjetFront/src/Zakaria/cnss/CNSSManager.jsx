...
import CNSSTable from "./CNSSTable";
...
function CNSSManager() {
  ...
  const [isAddingCNSS, setIsAddingCNSS] = useState(false);
  ...
  // Référence vers le composant CNSSTable
  const cnssTableRef = useRef(null);
  ...
  useEffect(() => {
    setTitle("Affiliations CNSS");
    setOnPrint(() => () => { if (cnssTableRef.current) cnssTableRef.current.handlePrint(); });
    setOnExportPDF(() => () => { if (cnssTableRef.current) cnssTableRef.current.exportToPDF(); });
    setOnExportExcel(() => () => { if (cnssTableRef.current) cnssTableRef.current.exportToExcel(); });
    return () => {
      clearActions();
    };
