...
class CnssDeclarationController extends Controller
{
    ...
    {
        try {
            $declarations = CnssDeclaration::with(['details.employe'])->withCount('details')
                ->orderBy('annee', 'desc')
                ->orderBy('mois', 'desc')
                ->orderBy('id', 'desc')
                ->get()
                ->map(function (CnssDeclaration $declaration) {
                    $montantTotal = (float) $declaration->montant_total;
                    $masseSalariale = round($declaration->details->sum(function (CnssDeclarationDetail $detail) use ($declaration) {
                        if (!$detail->employe) {
                            return 0;
                        }

                        return $this->resolveEmployeeSalary(
                            $detail->employe,
                            (int) $declaration->mois,
                            (int) $declaration->annee
                        );
                    }), 2);
    ...
    private function persistDeclaration(?CnssDeclaration $declaration, Request $request, Collection $selectedEmployeeIds)
    {
        try {
            ...
            $montantTotal = round($selectedEmployees->sum(function ($employee) {
                $salaire = (float) ($employee['salaire'] ?? 0);
                $cotisations = DeclarationIndividuelleCnss::calculerCotisations($salaire);

                return (float) $cotisations['cotisation_salarie'] + (float) $cotisations['cotisation_patronale'];
            }), 2);
