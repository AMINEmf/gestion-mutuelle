...
class CnssDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Vérifier si la table declarations_cnss existe
        $declarationsEnAttente = 0;
        $declarationsDeclare = 0;
        $declarationsPaye = 0;
        $montantCnss = 0;
        $dernieresDeclarations = collect();

        if (DB::getSchemaBuilder()->hasTable('declarations_cnss')) {
            ...
            $dernieresDeclarations = DB::table('declarations_cnss')
                ...
                ->map(function ($row) {
                    ...
                    $masseSalarialeRow = round((float) DB::table('declarations_individuelles_cnss')
                        ->where('mois', (int) $row->mois)
                        ->where('annee', (int) $row->annee)
                        ->sum('salaire_brut_imposable'), 2);
        ...
        $masseSalariale = round((float) DB::table('declarations_individuelles_cnss')
            ->where('mois', $mois)
            ->where('annee', $annee)
            ->sum('salaire_brut_imposable'), 2);
