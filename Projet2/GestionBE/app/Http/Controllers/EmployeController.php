...
'adresse.codePays' => 'nullable|string|max:10',
    ...
    /**
     * Liste légère pour les sélecteurs frontend (carrières/compétences).
     */
    public function listForSelect()
    {
        $employes = Employe::query()
            ...
            ->with([
                'departement:id,nom',
                'departements:id,nom',
                'manager:id,nom,prenom,matricule',
    ...
    public function getCompetences($id)
    {
        ...
        $rows = $employe->competences->map(function ($c) {
            return [
                'id' => $c->id,
                'nom' => $c->nom,
    ...
    public function addCompetence(Request $request, $id)
    {
        ...
        $dateAcquisition = $data['date_acquisition'] ?? now()->toDateString();
    ...
    /**
     * Met à jour le niveau d'une compétence pour un employé.
     */
    public function updateCompetence(Request $request, $id, $competenceId)
    {
        ...
        $employe = Employe::findOrFail($id);
        GpCompetence::findOrFail($competenceId);
        ...
        return response()->json(['message' => 'Compétence mise à jour avec succès.']);
    }
    ...
    /**
     * Remplace en masse les compétences d'un employé.
     */
    public function updateCompetences(Request $request, $id)
    {
        $data = $request->validate([
            'competences' => ['required', 'array'],
            'competences.*.competence_id' => ['required', 'exists:gp_competences,id'],
            'competences.*.niveau' => ['required', 'integer', 'min:1', 'max:5'],
            'competences.*.date_acquisition' => ['nullable', 'date'],
        ]);

        $employe = Employe::findOrFail($id);

        $syncPayload = [];
        foreach ($data['competences'] as $item) {
            $syncPayload[$item['competence_id']] = [
                'niveau' => $item['niveau'],
                'niveau_acquis' => $item['niveau'],
                'date_acquisition' => $item['date_acquisition'] ?? now()->toDateString(),
            ];
        }

        $employe->competences()->sync($syncPayload);
